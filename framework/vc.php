...
if(!function_exists('pts_VC_setup')) {
    function pts_VC_setup(){
        ...
        /*function vc_theme_vc_row($atts, $content = null) {
			...
			$customclass = ! empty( $custom ) ? $custom : 'vc_row-fluid';
			$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_row wpb_row '. $customclass .' '.set_container(). $el_class . vc_shortcode_custom_css_class( $css, ' ' ),  $atts );
	   ...
	   $setting_vc_single_image = array(
		...
		// **********************************************************************//
		// ! Register New Element: PTS login
		// **********************************************************************//
		...
		$login_params = array(
			...
			'params' => array(
				array(
					'type' => 'textfield',
					"heading" => __("Login Redirect", PTS_DOMAIN),
					"param_name" => "redirect_url",
					"description" => __("URL to redirect to when logging in", PTS_DOMAIN)
				),
				array(
					'type' => 'textfield',
					"heading" => __("Register URL", PTS_DOMAIN),
					"param_name" => "register_url",
					"description" => __("URL to the registration page. If set it over-rides the settings in options.", PTS_DOMAIN)
				)
			)
